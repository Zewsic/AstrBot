...
- **Качественная русская локализация панели управления** — весь русский перевод WebUI переписан заново с единой терминологией; закрыты места, где интерфейс показывал китайский текст или `[MISSING: ...]`, в том числе раздел настроек Telegram, добавленный этим форком. Ключи и подстановки в `zh-CN`, `en-US` и `ru-RU` синхронизированы полностью.
- **Локализация сообщений бота** — ошибки и ответы, которые бот отправляет пользователю (сбои LLM, вызовы инструментов, встроенные команды `/help`, `/reset`, `/new`, `/provider`, `/sid` и другие), больше не захардкожены на китайском или английском, а берутся из ресурсов `astrbot/core/i18n/locales/`. Язык выбирается автоматически по языку пользователя, сообщённому мессенджером (например, `language_code` в Telegram), либо принудительно — новой настройкой «Язык ответов» в разделе «Платформы → Основные»; по умолчанию используется английский.
