...
class TelegramPlatformAdapter(Platform):
    ...
    def _build_application(self) -> None:
        ...
        self.application.add_handler(
            TypeHandler(Update, self.guest_message_handler), group=-1
        )
    ...
    async def guest_message_handler(
        ...
        """Convert a Guest Mode update into an AstrBot event."""
        guest_message = getattr(update, "guest_message", None)
        if not guest_message:
            return
        if not self._guest_messages_enabled():
            logger.info("[Telegram] Ignored Guest Mode update because it is disabled.")
            raise ApplicationHandlerStop
        logger.info("[Telegram] Received Guest Mode update.")
        guest_update = Update(update_id=update.update_id, message=guest_message)
        abm = await self.convert_message(guest_update, context)
        if abm:
            abm.raw_message = update
            guest_chat = guest_message.chat
            guest_chat_name = (
                getattr(guest_chat, "title", None)
                or " ".join(
                    part
                    for part in [
                        getattr(guest_chat, "first_name", None),
                        getattr(guest_chat, "last_name", None),
                    ]
                    if part
                )
                or str(guest_chat.id)
            )
            abm.group = Group(
                group_id=str(guest_chat.id),
                group_name=guest_chat_name,
            )
            abm.telegram_guest_query_id = guest_message.guest_query_id
            abm.telegram_guest_message = True
            abm.telegram_guest_chat_context = {
                "id": str(guest_chat.id),
                "name": guest_chat_name,
                "type": str(guest_chat.type),
            }
            abm.session_id = f"guest:{guest_chat.id}:{guest_message.from_user.id}"
            await self.handle_msg(abm)
        raise ApplicationHandlerStop

    def _guest_messages_enabled(self) -> bool:
        """Return whether Telegram Guest Mode updates should be processed."""
        return (
            astrbot_config.get("platform_specific", {})
            .get("telegram", {})
            .get("enable_guest_messages", True)
        )
    ...
    async def convert_message(
        ...
        message.sender = MessageMember(
            ...
            language_code=_from_user.language_code,
        )
    ...
    def create_event(self, message: AstrBotMessage) -> TelegramPlatformEvent:
        ...
        return TelegramPlatformEvent(
            ...
            guest_query_id=getattr(message, "telegram_guest_query_id", None),
            is_guest_message=getattr(message, "telegram_guest_message", False),
            guest_chat_context=getattr(message, "telegram_guest_chat_context", None),
        )
